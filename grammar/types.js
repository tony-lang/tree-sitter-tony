...
const { typeParameters, commaSep1, struct, tuple, member, typeArguments } = require('./util')
...
module.exports = {
  ...
  parametric_type: ($) =>
    prec.right(seq(field('name', $.type), optional(field('arguments', typeArguments($))))),
  ...
  member_type: ($) => member($, $._type, $._type),

  tuple_type: ($) => tuple($, $._type),
  ...
  type_declaration: ($) =>
    prec.left(seq(field('name', $.type), optional(field('parameters', typeParameters($))))),
